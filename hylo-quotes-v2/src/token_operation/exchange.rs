...
use hylo_idl::tokens::{
  TokenMint, CBBTC, HYLOSOL, HYPE, HYUSD, JITOSOL, USDC, XBTC, XHYPE, XSOL,
};
...
//
// Each family is a generic helper bound on [`Exo`] plus one concrete impl per
// collateral, generated by `exo_routes!`. A blanket impl over `E: Exo` would
// overlap the LST impls above, so the concrete impls delegate instead.

/// Mint stablecoin (HYUSD) from exo collateral.
fn exo_mint_stable<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<E::Exp>,
) -> Result<OperationOutput<E::Exp, N6, N9>> {
  let exo = state.exo_exchange_context::<E>()?;
  ensure!(
    exo.stablecoin_mint_enabled(),
    "Exo stablecoin mint disabled"
  );
  let collateral_n9 = E::to_n9(in_amount)?;
  let FeeExtract {
    fees_extracted,
    amount_remaining,
...
/// Redeem stablecoin (HYUSD) for exo collateral.
fn exo_redeem_stable<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<N6>,
) -> Result<OperationOutput<N6, E::Exp, N9>> {
  let exo = state.exo_exchange_context::<E>()?;
  let stablecoin_nav = exo.stablecoin_nav()?;
  ...
  let out_amount = E::from_n9(amount_remaining)?;
  Ok(OperationOutput {
    in_amount,
    out_amount,
    fee_amount: fees_extracted,
...
/// Mint levercoin from exo collateral.
fn exo_mint_lever<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<E::Exp>,
) -> Result<OperationOutput<E::Exp, N6, N9>> {
  let exo = state.exo_exchange_context::<E>()?;
...
/// Redeem levercoin for exo collateral.
fn exo_redeem_lever<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<N6>,
) -> Result<OperationOutput<N6, E::Exp, N9>> {
  let exo = state.exo_exchange_context::<E>()?;
...
/// Swap stablecoin (HYUSD) to exo levercoin.
fn exo_stable_to_lever<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<N6>,
) -> Result<SwapOperationOutput> {
  let exo = state.exo_exchange_context::<E>()?;
  ...
  let out_amount = exo.swap_conversion()?.stable_to_lever(amount_remaining)?;
  Ok(OperationOutput {
    in_amount,
    out_amount,
    fee_amount: fees_extracted,
...
/// Swap exo levercoin to stablecoin (HYUSD).
fn exo_lever_to_stable<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<N6>,
) -> Result<SwapOperationOutput> {
  let exo = state.exo_exchange_context::<E>()?;
...
/// Swap exo collateral for USDC off the rebalance buy curve.
fn exo_to_usdc<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<E::Exp>,
) -> Result<OperationOutput<E::Exp, N6, E::Exp>> {
  let normalized = E::to_n9(in_amount)?;
  let usdc_price = state.usdc_exchange_state().usdc_usd_price;
  let conversion = state
    .exo_exchange_context::<E>()?
...
/// Swap USDC for exo collateral off the rebalance sell curve.
fn usdc_to_exo<E: Exo, C: SolanaClock>(
  state: &ProtocolState<C>,
  in_amount: UFix64<N6>,
) -> Result<OperationOutput<N6, E::Exp, N6>> {
  ...
  let collateral_out: UFix64<N9> = conversion.usdc_to_collateral(normalized)?;
  let out_amount = E::from_n9(collateral_out)?;
  Ok(OperationOutput {
    in_amount,
    out_amount,
    fee_amount: UFix64::zero(),
...
/// Generates the eight `TokenOperation` impls for one exo collateral pair.
macro_rules! exo_routes {
  ($exo:ident, $lever:ident) => {
    impl<C: SolanaClock> TokenOperation<$exo, HYUSD> for ProtocolState<C> {
      type FeeExp = N9;

      fn compute_output(
        &self,
        in_amount: UFix64<<$exo as TokenMint>::Exp>,
      ) -> Result<OperationOutput<<$exo as TokenMint>::Exp, N6, N9>> {
        exo_mint_stable::<$exo, C>(self, in_amount)
      }
    }

    impl<C: SolanaClock> TokenOperation<HYUSD, $exo> for ProtocolState<C> {
      type FeeExp = N9;

      fn compute_output(
        &self,
        in_amount: UFix64<N6>,
      ) -> Result<OperationOutput<N6, <$exo as TokenMint>::Exp, N9>> {
        exo_redeem_stable::<$exo, C>(self, in_amount)
      }
    }
    ...
    impl<C: SolanaClock> TokenOperation<$exo, $lever> for ProtocolState<C> {
      ...
      fn compute_output(
        ...
        exo_mint_lever::<$exo, C>(self, in_amount)
      }
    }
    ...
    impl<C: SolanaClock> TokenOperation<$lever, $exo> for ProtocolState<C> {
      ...
      fn compute_output(
        ...
        exo_redeem_lever::<$exo, C>(self, in_amount)
      }
    }
    ...
    impl<C: SolanaClock> TokenOperation<HYUSD, $lever> for ProtocolState<C> {
      ...
      fn compute_output(
        ...
        exo_stable_to_lever::<$exo, C>(self, in_amount)
      }
    }
    ...
    impl<C: SolanaClock> TokenOperation<$lever, HYUSD> for ProtocolState<C> {
      ...
      fn compute_output(
        ...
        exo_lever_to_stable::<$exo, C>(self, in_amount)
      }
    }

    impl<C: SolanaClock> TokenOperation<$exo, USDC> for ProtocolState<C> {
      type FeeExp = <$exo as TokenMint>::Exp;
      ...
      fn compute_output(
        ...
        OperationOutput<
          <$exo as TokenMint>::Exp,
          N6,
          <$exo as TokenMint>::Exp,
        >,
      > {
        exo_to_usdc::<$exo, C>(self, in_amount)
      }
    }

    impl<C: SolanaClock> TokenOperation<USDC, $exo> for ProtocolState<C> {
      type FeeExp = N6;

      fn compute_output(
        &self,
        in_amount: UFix64<N6>,
      ) -> Result<OperationOutput<N6, <$exo as TokenMint>::Exp, N6>> {
        usdc_to_exo::<$exo, C>(self, in_amount)
      }
    }
  };
}

exo_routes!(CBBTC, XBTC);
exo_routes!(HYPE, XHYPE);
